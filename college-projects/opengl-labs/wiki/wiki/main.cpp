...
void Display()
{
    if (gameOver == 0)
    {
        ...
        for (i = 0; i < AMMO; i++)
        {
            ...
            {
                glLoadIdentity();
                glColor3d((double)0 / 255, (double)0 / 255,
                    (double)0 / 255);
    ...
    glColor3d(0, 0, 0);
    printString((char*)" POWER", 6, 0.79, 0.04, 32);
    printString((char*)" POWER", 6, 0.79, -0.96, 32);
    printString((char*)"COMPUTER", 8, 0.812, 0.94, 26);
    printString((char*)" YOU", 4, 0.816, -0.06, 32);
    if (gameOver == 2)
    {
        glClear(GL_COLOR_BUFFER_BIT);
        glColor3d(0.0f, 0.0f, 0.0f);
        printString((char*)"YOU WON", 7, -0.25, 0, 80);
    }
    else if (gameOver == 3)
    {
        glClear(GL_COLOR_BUFFER_BIT);
        ...
        printString((char*)"COMPUTER WON", 12, -0.45, 0, 80);
    }
    else if (gameOver == 10)
    {
        glColor3d((double)0 / 255, (double)255 / 255, (double)11 / 255);
        
        glClear(GL_COLOR_BUFFER_BIT);
        printString((char*)"BATTLESHIPS", 14, -0.7, 0.8, 150);
        printString((char*)"Developers : Abhishek Mahajan, Kalrav Pavadia",
            strlen((char*)"Developers : Abhishek Mahajan, Kalrav Pavadia"), -1.0, 0.6, 60);
        printString((char*)"Submitted to : Aarzoo Mam",
            strlen("Submitted to : Aarzoo Mam"), -1.0, 0.4, 60);
        printString((char*)"Controls : ", strlen("Controls : "), -1.0, 0.2, 60);
        printString((char*)"1) w : Front", strlen("1) w : Front"), -0.63, 0.1, 60);
        printString((char*)"2) s : Back", strlen("2) s : Back"), -0.63, 0.0, 60);
        printString((char*)"3) a : Left", strlen("3) a : Left"), -0.63, -0.1, 60);
        printString((char*)"4) d : Right", strlen("4) d : Right"), -0.63, -0.2, 60);
        printString((char*)"5) Space : Fire", strlen("5) Space : Fire"), -0.63, -0.3,
            60);
        printString((char*)"6) Esc : Exit", strlen("6) Esc : Exit"), -0.63, -0.4, 60);
        printString((char*)"Game : Kill the Computer Ship.",
            strlen("Game : Kill the Computer Ship."), -1.0, -0.6, 60);
        printString((char*)"Press Enter to play game.",
