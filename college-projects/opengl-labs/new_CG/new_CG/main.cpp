...
char college[100] = "ASSGINMENT -1";
char dept[100] = "COMPUTER GRAPHICS(UCS505)";
char heading[100] = "Made by";
char row1[100] = "Anandi Bali (102003436)";
char row2[100] = "Rhea Seth (102053013)";
...
void frontscreen(void)
{
    ...
    for (s = 0; dept[s] != '\0'; s++)
    {
        glColor3f(1.0, 0.0, 0.0); glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, dept[s]);
    }
    glRasterPos2i(210, 380); //displays project name
    for (s = 0; title[s] != '\0'; s++)
    {
        glColor3f(0.0, 0.0, 0.0); glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, title[s]);
    }
    ...
    {
        glColor3f(0.0, 0.0, 0.0); glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, heading[s]);
    }
    ...
    for (s = 0; row1[s] != '\0'; s++)
    {
        glColor3f(0.0, 0.0, 0.0); glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, row1[s]);
    }
    glRasterPos2i(170, 290); //displays 2nd row in table Code 6
    
    for (s = 0; row2[s] != '\0'; s++)
    {
        glColor3f(0.0, 0.0, 0.0); glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, row2[s]);
    }
    ...
    for (s = 0; emsg[s] != '\0'; s++)
    {
        glColor3f(1.0, 0.0, 0.5); glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, emsg[s]);
    }
    glPopMatrix();
    glFlush();
...
void Drawarc(float sa, float ea, float cx, float cy, float rd)
    ...
    for (angle = sa; angle < ea; angle += step)
    {
        ...
        y = centerY + radius * sin(rad); glVertex2f(x, y);
    }
    glEnd();
    glFlush();
}
...
glColor3f(0.2 + col, 0.2 + col, 0.2 + col);
    glBegin(GL_POLYGON); glVertex2f(10, 119);
glVertex2f(10, 110);
glVertex2f(41, 70);
...
glBegin(GL_POLYGON); glVertex2f(51, 151);
glVertex2f(51, 140);
glVertex2f(221, 149);
...
 
        ...
        void flag(float x)
        {
            ...
            {
                //displays college name
                glColor3f(1.0, 1.0, 1.0); glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, name[s]);
            }
            glPopMatrix();
        }
        void crackers()
        ...
        void plane() {
            ...
            glEnd();
            glColor3f(0.0, 0.0, 0.0);
            glBegin(GL_LINE_LOOP);//outline of upper triangle planre
            glVertex2f(135.0 / 3, 55.0 / 3);
            glVertex2f(150.0 / 3, 50.0 / 3);
            glVertex2f(155.0 / 3, 45.0 / 3);
            glVertex2f(160.0 / 3, 40.0 / 3);
            glVertex2f(135.0 / 3, 40.0 / 3);
            glEnd();
            glColor3f(1.0, 0.0, 0.0); glBegin(GL_POLYGON);//lower triangle
            glVertex2f(135.0 / 3, 40.0 / 3);
            glVertex2f(160.0 / 3, 40.0 / 3);
